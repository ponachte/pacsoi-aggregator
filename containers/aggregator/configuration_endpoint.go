...
func startConfigurationEndpoint(mux *http.ServeMux) *ConfigurationData {
	...
	configurationData.HandleFunc("/config", configurationData.HandleConfigurationEndpoint, []auth.Scope{auth.READ})
	configurationData.HandleFunc("/config/actors", configurationData.HandleActorsEndpoint, []auth.Scope{auth.READ, auth.CREATE})

	return &configurationData
}

func (data ConfigurationData) HandleFunc(pattern string, handler func(http.ResponseWriter, *http.Request), resourceDescription []auth.Scope) {
	data.serveMux.HandleFunc(pattern, handler)
	auth.CreateResource(
		fmt.Sprintf("%s:%s%s", externalHost, externalPort, pattern),
		resourceDescription,
	)
}
...
func (data ConfigurationData) createActor(response http.ResponseWriter, request *http.Request) {
	...
	data.HandleFunc(fmt.Sprintf("/config/actors/%s", actor.Id), data.HandleActorEndpoint, []auth.Scope{auth.DELETE, auth.READ})
...
func (data ConfigurationData) deleteActor(response http.ResponseWriter, _ *http.Request, actor Actor) {
	...
	auth.DeleteResource(
		fmt.Sprintf("%s:%s/config/actors/%s", externalHost, externalPort, actor.Id),
	)
}
